...
const typeLabels = {
  supplier_concentration: 'Concentracao em fornecedor',
  above_average_spending: 'Acima da media da base',
  sensitive_category_share: 'Categoria sensivel',
};
      ...
      <Card className="border-green-200 bg-green-50">
        <CardContent className="p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-bold text-green-950">Nenhum ponto de atencao encontrado</h3>
              <p className="text-sm text-green-800">
                Pelos criterios atuais do FISCALIZA, nao houve alerta de concentracao em fornecedor ou gasto acima da media sincronizada para este ano.
              </p>
            </div>
          </div>
        </CardContent>
    ...
    <Card className="border-yellow-200 bg-yellow-50">
      <CardContent className="p-5">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            <h3 className="font-bold text-yellow-950">Pontos de atencao neste perfil</h3>
            <p className="text-sm text-yellow-800">
              Estes sinais orientam leitura da fonte oficial. Eles nao indicam irregularidade sozinhos.
            </p>
          </div>
        </div>
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2 mb-2">
                    <span className={`rounded-full border px-2 py-0.5 text-xs font-bold ${levelStyles[point.level] || levelStyles.medium}`}>
                      {point.level === 'high' ? 'Atencao alta' : 'Atencao media'}
                    </span>
                    ...
                    <div className="rounded-lg bg-gray-50 p-3">
                      ...
                      {point.recordCount !== undefined && (
                        <p className="mt-1 text-xs text-gray-500">
                          {point.recordCount === null ? 'Registros: nao informado' : `${point.recordCount} registro${point.recordCount === 1 ? '' : 's'}`}
                        </p>
                      )}
                    </div>
                  </div>
