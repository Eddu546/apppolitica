          ...
          <div className="flex items-start gap-4 flex-1">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2 mb-1">
                <h2 className={`text-lg font-black ${style.title}`}>{seal.title}</h2>
                <span className={`rounded-full border px-2.5 py-1 text-xs font-bold ${style.pill}`}>
                  {style.label}
                </span>
              </div>
              <p className={`text-sm leading-relaxed ${style.text}`}>{seal.description}</p>
              <p className="mt-2 text-xs text-slate-600">
                O selo e calculado pelo FISCALIZA a partir de fontes oficiais. Ele nao e uma declaracao da Camara e nao avalia outras despesas do mandato.
              </p>
            </div>
          </div>
              ...
              return (
                <div key={check.id} className="rounded-lg border border-white/70 bg-white p-4 shadow-sm">
                  ...
                  <div className="mt-3 border-t border-slate-100 pt-3 text-[11px] text-slate-500 space-y-1">
                    <p><strong>Fonte:</strong> {check.sourceName || 'Fonte nao informada'}</p>
                    ...
                    <details>
                      ...
                      <p className="mt-1 leading-relaxed">{check.calculationMethod || 'Metodo nao informado.'}</p>
                    </details>
                    {check.sourceUrl && (
                      <a
